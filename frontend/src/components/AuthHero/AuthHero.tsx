...
export function AuthHero({ variant }: AuthHeroProps) {
  if (variant === 'banner') {
    return (
      <div className="relative overflow-hidden bg-linear-to-r from-auth-primary via-auth-primary-hover to-[#1e3a8a] px-5 py-4 text-white">
        <div
          className="pointer-events-none absolute -left-12 -top-12 h-32 w-32 rounded-full bg-white/10 blur-2xl"
          aria-hidden
        />
        <div
          className="pointer-events-none absolute -bottom-16 right-0 h-40 w-40 rounded-full bg-white/10 blur-2xl"
          aria-hidden
        />
  ...
  return (
    <div className="relative flex h-full min-h-40 min-w-0 flex-col justify-between overflow-hidden bg-linear-to-br from-auth-primary via-auth-primary-hover to-[#172554] px-8 pb-7 pt-7 text-white lg:min-h-0 lg:px-10 lg:pb-9 lg:pt-9">
      <div
        className="pointer-events-none absolute -left-24 top-1/4 h-64 w-64 -translate-y-1/2 rounded-full bg-sky-300/20 blur-3xl"
        aria-hidden
      />
      <div
        className="pointer-events-none absolute -right-12 bottom-0 h-72 w-72 rounded-full bg-indigo-400/15 blur-3xl"
        aria-hidden
      />
      <div
      ...
      <div className="relative mt-8 flex items-end gap-3 lg:mt-10">
        <div className="flex -space-x-2">
          {[0, 1, 2].map((i) => (
            <span
              key={i}
              className="h-9 w-9 rounded-full border-2 border-white/30 bg-white/20 ring-2 ring-auth-primary"
            />
          ))}
        </div>
